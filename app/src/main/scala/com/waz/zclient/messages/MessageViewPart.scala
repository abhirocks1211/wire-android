...
class UserPartView(context: Context, attrs: AttributeSet, style: Int) extends LinearLayout(context, attrs, style) with MessageViewPart with ViewHelper {
  ...
  user.map(u => if (u.isWireBot) u.name else if (u.deleted) Name(getString(R.string.user_deleted_username)) else u.getDisplayName).onUi(tvName.setTransformedText(_))
